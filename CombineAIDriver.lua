...
function CombineAIDriver:driveFieldworkUnloadOrRefill()
	...
	elseif self.fieldworkUnloadOrRefillState == self.states.WAITING_FOR_UNLOAD_IN_POCKET or
		...
		if self:unloadFinished() then
			...
			if self.fieldworkUnloadOrRefillState == self.states.WAITING_FOR_UNLOAD_BEFORE_STARTING_NEXT_ROW and
				not self.waitingForUnloaderAtEndOfRow:get() then
				self:cancelRendezvous()
...
function CombineAIDriver:getUnloaderRendezvousWaypoint(unloaderEstimatedSecondsEnroute, unloadAIDriver)
	...
	self:debug('Rendezvous request: seconds until full: %d, unloader ETE: %d (around my wp %d), full at waypoint %d, ',
			self.secondsUntilFull or -1, unloaderEstimatedSecondsEnroute, unloaderRendezvousWaypointIx, 
			self.waypointIxWhenFull or -1)

	-- rendezvous at whichever is closer
	unloaderRendezvousWaypointIx = math.min(unloaderRendezvousWaypointIx, self.waypointIxWhenFull)
...
function CombineAIDriver:findBestWaypointToUnloadOnUpDownRows(ix)
	...
	if pipeInFruit then
		...
		if ixAtRowStart and lRow - dToNextTurn < 20 then
			newWpIx = math.max(ixAtRowStart + 1, ix - 4)
		end
	end
